-- shameless plug of https://github.com/olexsmir/gopher.nvim/blob/main/lua/gopher/_utils/ts/nodes.lua
...
function M.nodes_at_cursor(query, default, bufnr, row, col)
  ...
  if nodes == nil then
    logger.debug("Unable to find any nodes. Place your cursor on a go symbol and try again")
    return nil
  end

  nodes = M.sort_nodes(M.intersect_nodes(nodes, row, col))
  if nodes == nil or #nodes == 0 then
    logger.debug("Unable to find any nodes at pos. " .. tostring(row) .. ":" .. tostring(col))
    return nil
  end
